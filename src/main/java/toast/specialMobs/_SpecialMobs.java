...
public class _SpecialMobs {
    ...
    public static final String[] MONSTER_KEY = { "Blaze", "CaveSpider", "Creeper", "Enderman", "Ghast", "PigZombie",
            "Silverfish", "Skeleton", "Slime", "Spider", "Witch", "Zombie" };
    /** Monster "sub-species" array. First dimension is the MONSTER_KEY[]. */
    public static final String[][] MONSTER_TYPES = { { // Blaze
            "Cinder", "Conflagration", "Ember", "Hellfire", "Inferno", "Jolt", "Smolder", "Wildfire" },
            { // CaveSpider
                    "Baby", "Flying", "Mother", "Tough", "Web", "Witch" },
            { // Creeper
                    "Armor", "Dark", "Death", "Dirt", "Doom", "Drowning", "Ender", "Fire", "Gravel", "Gravity",
                    "Jumping", "Lightning", "Mini", "Splitting" },
            { // Enderman
            ...
            { // Spider
                    "Baby", "Desert", "Flying", "Ghost", "Giant", "Hungry", "Mother", "Pale", "Poison", "Small",
                    "Tough", "Web", "Witch" },
            { // Witch
    ...
    public static final int[] MONSTER_KEY_COLORS = {
            /*
             * "Blaze", "CaveSpider", "Creeper", "Enderman", "Ghast", "PigZombie", "Silverfish", "Skeleton", "Slime",
             * "Spider", "Witch", "Zombie"
             */
            0xf6b201, 0x0c424e, 0x0da70b, 0x161616, 0xf9f9f9, 0xea9393, 0x6e6e6e, 0xc1c1c1, 0x51a03e, 0x342d27,
            0x340000, 0x00afaf };
    ...
    public static final int[][] MONSTER_TYPE_COLORS = { { /*
                                                           * Blaze (0xfff87e) "Cinder", "Conflagration", "Ember",
                                                           * "Hellfire", "Inferno", "Jolt", "Smolder", "Wildfire"
                                                           */
            0xffc0cb, 0xfff87e, 0x000000, 0xdddddd, 0xf14f00, 0x499cae, 0x000000, 0xf4ee32 },
            { /*
               * CaveSpider (0xa80e0e) "Baby", "Flying", "Mother", "Tough", "Web", "Witch"
               */
                    0xffc0cb, 0x6388b2, 0xb300b3, 0x8ea80e, 0xe7e7e7, 0xdd0e0e },
            { /*
               * Creeper (0x000000) "Armor", "Dark", "Death", "Dirt", "Doom", "Drowning", "Ender", "Fire", "Gravel",
               * "Gravity", "Jumping", "Lightning", "Mini", "Splitting"
               */
                    0xc39536, 0xf9ff3a, 0xcd0000, 0x78553b, 0x494949, 0x2d41f4, 0xcc00fa, 0xe13916, 0x908884, 0x220022,
                    0x7d6097, 0x499cae, 0xffc0cb, 0x5f9d22 },
            { /*
               * Enderman (0x000000) "Blinding", "Cursed", "Icy", "Lightning", "Mini", "Mirage", "Thief"
               */
                    0xffffff, 0xab1d1d, 0x72959c, 0x4bb4b5, 0xffc0cb, 0xc2bc84, 0x04fa00 },
            { /*
               * Ghast (0xbcbcbc) "Baby", "Faint", "Fighter", "King", "Mini", "Queen", "Unholy"
               */
                    0xffc0cb, 0x82c873, 0x7a1300, 0xe8c51a, 0xbcbcbc, 0xce0aff, 0x7ac754 },
            { /*
               * PigZombie (0x4c7129) "Brutish", "Fishing", "Giant", "Hungry", "Plague", "Vampire"
               */
                    0xfff87e, 0x2d41f4, 0x4c7129, 0xab1518, 0x8aa838, 0x000000 },
            { /*
               * Silverfish (0x303030) "Blinding", "Fishing", "Flying", "Poison", "Tough"
               */
                    0x000000, 0x2d41f4, 0x6388b2, 0x779c68, 0xdd0e0e },
            { /*
               * Skeleton (0x494949) "Brutish", "Fire", "Gatling", "Giant", "Ninja", "Poison", "Sniper", "Spitfire",
               * "Thief"
               */
                    0xfff87e, 0xdc1a00, 0xffff0b, 0x494949, 0x333366, 0x779c68, 0x486720, 0xdc1a00, 0x000000 },
            { /*
               * Slime (0x7ebf6e) "Blackberry", "Blueberry", "Caramel", "Grape", "Lemon", "Strawberry", "Watermelon"
               */
                    0x331133, 0x766bbc, 0x9d733f, 0xb333b3, 0xe6e861, 0xbe696b, 0xdf7679 },
            { /*
               * Spider (0xa80e0e) "Baby", "Desert", "Flying", "Ghost", "Giant", "Hungry", "Mother", "Pale", "Poison",
               * "Small", "Tough", "Web", "Witch"
               */
                    0xffc0cb, 0xe6ddac, 0x6388b2, 0x82c873, 0xa80e0e, 0x799c65, 0xb300b3, 0xded4c6, 0x0c424e, 0xa80e0e,
                    0x8ea80e, 0xe7e7e7, 0xdd0e0e },
            { /*
               * Witch (0x51a03e) "Domination", "Rage", "Shadows", "Undead", "Wilds", "Wind"
               */
                    0xfff87e, 0x932423, 0x000000, 0x799c65, 0xa80e0e, 0x6388b2 },
            { /*
               * Zombie (0x799c65) "Brutish", "Fire", "Fishing", "Giant", "Hungry", "Plague"
               */
    ...
    private void registerMobs() {
        ...
        for (int i = 0; i < _SpecialMobs.MONSTER_KEY.length; i++) {
            ...
            try {
                entityClass = Class.forName(
                        "toast.specialMobs.entity." + _SpecialMobs.MONSTER_KEY[i].toLowerCase()
                                + ".Entity_Special"
                                + _SpecialMobs.MONSTER_KEY[i]);
                EntityRegistry.registerModEntity(
                        entityClass,
                        "Special" + _SpecialMobs.MONSTER_KEY[i],
                        id++,
                        this,
                        80,
                        3,
            ...
            for (int j = 0; j < _SpecialMobs.MONSTER_TYPES[i].length; j++) {
                ...
                try {
                    entityClass = Class.forName(
                            "toast.specialMobs.entity." + _SpecialMobs.MONSTER_KEY[i].toLowerCase() + ".Entity" + name);
                    ...
                    if (makeSpawnEggs) {
                        ...
                        try {
                            if (eggIdClaimer != null) {
                                eggId = ((Integer) eggIdClaimer
                                        .invoke(EntityRegistry.instance(), Integer.valueOf(eggId))).intValue();
                            }
                        } catch (Exception ex) {
                            // Do nothing
                        }
        ...
        if (spawnWeight > 0) {
            EntityRegistry
                    .addSpawn(EntityEnderCreeper.class, spawnWeight, 1, 1, EnumCreatureType.monster, BiomeGenBase.sky);
        }
        spawnWeight = Properties.getInt(Properties.SPAWNING, "nether_fire_creeper");
        if (spawnWeight > 0) {
            EntityRegistry
                    .addSpawn(EntityFireCreeper.class, spawnWeight, 4, 4, EnumCreatureType.monster, BiomeGenBase.hell);
        }
        ...
        if (makeSpawnEggs) {
            ...
            try {
                ...
                _SpecialMobs.console(
                        "Error claiming spawn egg ID! Spawn egg will probably be overwritten. @"
                                + ex.getClass().getName());
            }
     ...
     * @return A length 2 int[] corresponding to the two indecies in MONSTER_TYPES[][] for the key and type,
     *         respectively.
     */
     ...
     * @Deprecated public static String[] parseName(String mobName) { char[] chars = mobName.toCharArray(); boolean[]
     * breaks = new boolean[chars.length]; String[] parts = new String[chars.length]; String mobType = ""; String mob =
     * ""; for (int i = 0; i < chars.length; i++) if (Character.isUpperCase(chars[i])) breaks[i] = true; int index = -1;
     * for (int i = 0; i < chars.length; i++) { if (breaks[i] == true) index++; if (index >= 0) { if (parts[index] ==
     * null) parts[index] = ""; parts[index] = parts[index] + Character.toString(chars[i]); } } for (int i = 0; i <
     * chars.length; i++) { if (parts[i] == null || parts[i] == "Entity") continue; if (mobType(parts[i]) >= 0) {
     * mobType = parts[i]; break; } if (i + 1 < chars.length && mobType(parts[i] + parts[i + 1]) >= 0) { mobType =
     * parts[i] + parts[i + 1]; break; } mob = mob + parts[i]; } return new String[] { mobType, mob }; }
     */
    ...
    public static String cap(String string) {
        ...
        if (length <= 0) return "";
        if (length == 1) return string.toUpperCase();
    ...
    public static String decap(String string) {
        int length = string.length();
        if (length <= 0) return "";
        if (length == 1) return string.toLowerCase();
     ...
     * @return A copy of the string with a space before each capital letter beyond the first, or "Zombie Pigman" if the
     *         given string was "PigZombie".
     */
    public static String localizeName(String name) {
        if (name.equals("PigZombie")) return "Zombie Pigman";
        if (name.length() > 1) {
    ...
    /**
     * Throws a runtime exception with a message and this mod's name tag if debugging is enabled. Otherwise, prints the
     * message to the console tagged as an error.
     *
     * @param message The message to print.
     */
    public static void debugException(String message) {
        if (_SpecialMobs.debug) throw new RuntimeException("[" + _SpecialMobs.MODID + "] " + message);
